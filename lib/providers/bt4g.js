...
class Bt4g extends TorrentProvider {
  ...
  formatMagnet(infoHash, name) {
    ...
    if (infoHash) {
      ...
      result = `magnet:?xt=urn:btih:${infoHash}&dn=${encodeURIComponent(name)}${trackersQueryString}`;
    }
    return result;
  }

  async getMagnet(torrent) {
    // TODO: xray extra attributes to get name...
    const torrentDetailUrl = await this.xray(encodeURI(torrent.desc), 'li a.grey-text.text-darken-4@href');
